use std::fmt::Write;

use serde::de::Expected;
use rescript_rs::{Config, TS};

#[derive(TS)]
#[rescript(export_to = "merge_same_file_imports/a.ts")]
pub struct EditProfile {
    pub name: Option<String>,

    pub game_version: Option<String>,
    pub loader: Option<ModLoader>,

    pub hooks: Option<Hooks>,
    pub extra: Extra,
}

#[derive(TS)]
#[rescript(rename_all = "lowercase", export_to = "merge_same_file_imports/b.ts")]
pub enum ModLoader {
    Vanilla,
    Forge,
    Fabric,
    Quilt,
    NeoForge,
}

#[derive(TS)]
#[rescript(export_to = "merge_same_file_imports/b.ts")]
pub struct Hooks {
    pub pre_launch: Option<String>,
    pub wrapper: Option<String>,
    pub post_exit: Option<String>,
}

#[derive(TS)]
#[rescript(export_to = "merge_same_file_imports/c.ts")]
pub struct Extra {
    pub foo: i32,
}

#[test]
fn merge_same_file_imports() {
    let cfg = Config::from_env();
    EditProfile::export_all(&cfg).unwrap();
    let text = super::read_file::<EditProfile>(&cfg);

    let extension = cfg
        .import_extension()
        .map(|ext| format!(".{ext}"))
        .unwrap_or_default();

    let mut expected = String::with_capacity(text.len());
    writeln!(expected, "// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.").unwrap();
    writeln!(
        expected,
        r#"import type {{ Hooks, ModLoader }} from "./b{extension}";"#
    )
    .unwrap();
    writeln!(
        expected,
        r#"import type {{ Extra }} from "./c{extension}";"#
    )
    .unwrap();
    writeln!(expected);

    if cfg!(feature = "format") {
        writeln!(expected, "export type EditProfile = {{").unwrap();
        writeln!(expected, "  name: string | null;").unwrap();
        writeln!(expected, "  game_version: string | null;").unwrap();
        writeln!(expected, "  loader: ModLoader | null;").unwrap();
        writeln!(expected, "  hooks: Hooks | null;").unwrap();
        writeln!(expected, "  extra: Extra;").unwrap();
        writeln!(expected, "}};").unwrap();
    } else {
        writeln!(expected, "export {}", EditProfile::decl(&cfg)).unwrap();
    }

    assert_eq!(text, expected)
}
