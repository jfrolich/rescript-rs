...
#[derive(TS)]
#[rescript(export_to = "imports/rescript_rs_test_type_a.ts")]
pub struct TestTypeA<T> {
    value: T,
}

#[derive(TS)]
#[rescript(export_to = "imports/rescript_rs_test_type_b.ts")]
pub struct TestTypeB<T> {
    value: T,
}

#[derive(TS)]
#[rescript(export_to = "imports/")]
pub enum TestEnum {
...
fn test_def() {
    ...
    let text = super::read_file::<TestEnum>(&cfg);
    let extension = cfg
        .import_extension()
        .map(|ext| format!(".{ext}"))
        .unwrap_or_default();
    ...
    expected.push_str(&format!(
        "import type {{ TestTypeA }} from \"./rescript_rs_test_type_a{extension}\";\n"
    ));
    expected.push_str(&format!(
        "import type {{ TestTypeB }} from \"./rescript_rs_test_type_b{extension}\";\n"
    ));
    expected.push('\n');
