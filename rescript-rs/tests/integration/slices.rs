...
#[derive(TS)]
#[rescript(export, export_to = "slices/")]
struct Interface {
    #[allow(dead_code)]
    a: [i32],
}
...
fn interface() {
    let cfg = Config::from_env();
    assert_eq!(Interface::inline(&cfg), "{ a: array<int>, }")
}
...
fn boxed_interface() {
    let cfg = Config::from_env();
    assert_eq!(InterfaceBoxed::inline(&cfg), "{ a: array<int>, }")
}
...
fn boxed_newtype() {
    let cfg = Config::from_env();
    assert_eq!(NewtypeBoxed::inline(&cfg), "array<int>")
}

#[derive(TS)]
#[rescript(export, export_to = "slices/nested/")]
struct InnerMost;
